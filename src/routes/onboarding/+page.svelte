<script lang="ts">
	...
	const onboardingModules = $state([
		{
			...
			estimatedTime: 15,
			lessons: [
				{ id: 'gs-1', title: 'Welcome to CodeReview.live', completed: true, duration: 3, action: null },
				{ id: 'gs-2', title: 'Creating Your First Project', completed: false, duration: 3, action: '/projects/new' },
				{ id: 'gs-3', title: 'Navigating the Dashboard', completed: false, duration: 2, action: '/dashboard' },
				{ id: 'gs-4', title: 'Understanding the Interface', completed: false, duration: 4, action: null },
				{ id: 'gs-5', title: 'Setting Up Your Profile', completed: false, duration: 3, action: '/onboarding/profile' }
			]
		},
		{
			id: 'video-reviews',
		...
		},
		{
			...
			estimatedTime: 20,
			lessons: [
				{ id: 'ca-1', title: 'Importing Code from GitHub', completed: false, duration: 4 },
				{ id: 'ca-2', title: 'Using the Diff Viewer', completed: false, duration: 4 },
	...
	$effect(() => {
		...
		if (auth.currentUser && !auth.currentUser.name) {
			goto('/onboarding/profile');
		}

		// Auto-complete profile lesson
		const gsModule = onboardingModules.find((m) => m.id === 'getting-started');
		if (gsModule) {
			// Mark "Setting Up Your Profile" as complete if we have a user name
			const profileLesson = gsModule.lessons.find((l) => l.id === 'gs-5');
			if (profileLesson && !profileLesson.completed && auth.currentUser?.name) {
				profileLesson.completed = true;
				gsModule.completedLessons++;
				saveProgress();
			}
		}
	});
	...
	onMount(() => {
		...
		if (browser) {
			...
			if (saved) {
				...
				onboardingModules.forEach((m) => {
					const savedModule = parsed.find((pm: any) => pm.id === m.id);
					if (savedModule) {
						m.lessons.forEach((l) => {
							const savedLesson = savedModule.lessons.find((pl: any) => pl.id === l.id);
							if (savedLesson) l.completed = savedLesson.completed;
						});
		...
		// Auto-complete "Welcome" lesson
		const gsModule = onboardingModules.find((m) => m.id === 'getting-started');
		if (gsModule) {
			const welcomeLesson = gsModule.lessons.find((l) => l.id === 'gs-1');
			if (welcomeLesson && !welcomeLesson.completed) {
				welcomeLesson.completed = true;
				gsModule.completedLessons++;
				saveProgress();
			}
		}
							...
							<div class="space-y-2">
								{#each module.lessons as lesson, i}
									...
									>
										...
										{#if !lesson.completed}
											<Button size="sm" onclick={() => handleLessonAction(module, lesson)}>
