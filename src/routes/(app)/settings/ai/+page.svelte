<script lang="ts">
	...
	const creditsLimit = $derived(
		getLimit(subscriptionsStore.current?.plan || 'free', 'aiCreditsPerMonth')
	);
...
<div class="space-y-6">
			...
			{#if settings.aiEnabled}
				<div class="space-y-4 pl-6 border-l-2">
					...
					<div class="flex items-center justify-between">
						...
						<Switch
							checked={settings.detectSmells}
							onCheckedChange={(checked) => updateSetting('detectSmells', checked)}
						/>
					</div>
					...
					<div class="flex items-center justify-between">
						...
						<Switch
							...
							onCheckedChange={(checked) => updateSetting('suggestImprovements', checked)}
						/>
					</div>
				</div>
			{/if}
			...
			<div class="space-y-2">
				...
				<div class="flex gap-2">
					<Input
						...
						class="flex-1"
					/>
					<Button variant="outline" onclick={testConnection}>Test</Button>
				</div>
			</div>
	...
	<Card>
		...
		<CardContent class="space-y-2">
			...
			<p class="text-xs text-muted-foreground">Resets on the 1st of each month</p>
		</CardContent>
	</Card>
