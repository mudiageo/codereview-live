...
<div class="flex h-screen overflow-hidden bg-background">
	...
	>
		...
		<div class="flex h-16 items-center gap-2 border-b px-4 md:px-6 animate-fade-in">
			<div
				class="flex h-8 w-8 items-center justify-center rounded-lg bg-primary text-primary-foreground shrink-0 animate-scale-in gradient-glow"
			>
				<Video class="h-5 w-5" />
			</div>
			{#if !sidebarCollapsed}
		...
		<nav class="flex-1 space-y-1 p-4 overflow-y-auto stagger-fade-in">
			{#each navigation as item}
				<a
					href={item.href}
					class={cn(
						'flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-all duration-200 hover-lift will-change-transform',
						isActive(item.href)
