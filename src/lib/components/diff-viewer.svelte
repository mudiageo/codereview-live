			...
			{#if viewMode === 'unified'}
				...
				<div class="font-mono text-xs">
					{#each parsedDiff() as line, i (i)}
						{@const annotations = line.lineNumber?.new
							? getLineAnnotations(line.lineNumber.new)
							: []}
						<div class="flex {getLineClass(line.type)} relative group">
							<button
								...
								onclick={() =>
									onLineClick && line.lineNumber?.new && onLineClick(line.lineNumber.new)}
								type="button"
							>
								{#if line.lineNumber}
							...
							<div
								class="flex-1 px-4 py-1 whitespace-pre-wrap break-all flex items-center justify-between"
							>
								<div class="flex-1">
								...
								<div class="flex items-center gap-1">
											...
											<PopoverTrigger>
												{#snippet child(props)}
													<button
														{...props}
														type="button"
														class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity {getAnnotationColor(
															annotations
														)}"
													>
													...
													{#each annotations as annotation}
														...
														<div class="text-sm">
															<div class="flex items-center gap-2 mb-1">
																...
																<span
																	class="font-semibold capitalize {getAnnotationColor(annotation)}"
																>
																	{annotation.type}
																</span>
									...
									{#if onExplainCode && line.content.trim() && line.type !== 'header'}
										<button
											...
											onclick={() =>
												line.lineNumber?.new && onExplainCode(line.lineNumber.new, line.content)}
			...
			{:else}
				...
				<div class="grid grid-cols-2 font-mono text-xs">
					<div class="border-r">
						{#each parsedDiff() as line, i (i)}
							{#if line.type !== 'add'}
								{@const annotations = line.lineNumber?.old
									? getLineAnnotations(line.lineNumber.old)
									: []}
								<div class="flex {getLineClass(line.type)} relative group">
									<button
										...
										onclick={() =>
											onLineClick && line.lineNumber?.old && onLineClick(line.lineNumber.old)}
										type="button"
									>
										{line.lineNumber?.old || ''}
									</button>
									...
									>
										...
										{#if onExplainCode && line.content.trim() && line.type !== 'header'}
											<button
												...
												class="ml-2 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-purple-100 dark:hover:bg-purple-900/20 rounded p-1"
												onclick={() =>
													line.lineNumber?.old && onExplainCode(line.lineNumber.old, line.content)}
												title="Explain this code with AI"
											>
												<Sparkles class="h-4 w-4 text-purple-600 dark:text-purple-400" />
											</button>
