    ...
    <Card class="overflow-hidden">
      <CardHeader class="pb-3 bg-gradient-to-r from-primary/5 to-transparent">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <Badge variant="outline" class={analysis.overallScore >= 80 ? 'bg-green-500/10 text-green-500' : analysis.overallScore >= 60 ? 'bg-yellow-500/10 text-yellow-500' : 'bg-red-500/10 text-red-500'}>
      ...
      <CardContent class="pt-4 space-y-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <button class="flex flex-col items-center p-3 rounded-lg bg-gradient-to-br from-red-500/10 to-red-600/5 border border-red-500/10 hover:border-red-500/30 transition-colors cursor-pointer" onclick={() => activeTab = 'bugs'}>
          ...
          <button class="flex flex-col items-center p-3 rounded-lg bg-gradient-to-br from-orange-500/10 to-orange-600/5 border border-orange-500/10 hover:border-orange-500/30 transition-colors cursor-pointer" onclick={() => activeTab = 'security'}>
            <ShieldAlert class="h-5 w-5 text-orange-500 mb-1" />
            <span class="text-lg font-bold text-orange-500">{analysis.securityIssues?.length || 0}</span>
            <span class="text-xs text-muted-foreground">Security</span>
          </button>
          <button class="flex flex-col items-center p-3 rounded-lg bg-gradient-to-br from-yellow-500/10 to-yellow-600/5 border border-yellow-500/10 hover:border-yellow-500/30 transition-colors cursor-pointer" onclick={() => activeTab = 'performance'}>
            <Zap class="h-5 w-5 text-yellow-500 mb-1" />
            <span class="text-lg font-bold text-yellow-500">{analysis.performanceNotes?.length || 0}</span>
            <span class="text-xs text-muted-foreground">Performance</span>
          </button>
          <button class="flex flex-col items-center p-3 rounded-lg bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 border border-emerald-500/10 hover:border-emerald-500/30 transition-colors cursor-pointer" onclick={() => activeTab = 'suggestions'}>
            <Lightbulb class="h-5 w-5 text-emerald-500 mb-1" />
            <span class="text-lg font-bold text-emerald-500">{analysis.suggestions?.length || 0}</span>
          ...
          <TabsContent value="bugs" class="mt-4 space-y-3">
            ...
            {:else}
              {#each analysis.bugs as bug}
                <div class="rounded-lg border overflow-hidden" class:border-red-500/30={bug.severity === 'critical' || bug.severity === 'high'}>
                  <button class="w-full p-3 flex items-start gap-3 text-left hover:bg-muted/30 transition-colors" onclick={() => toggleExpand(bug.id)}>
                    <Badge class={`${getSeverityColor(bug.severity)} shrink-0`}>{bug.severity}</Badge>
                    ...
                    {#if expandedItems.has(bug.id)}<ChevronUp class="h-4 w-4 text-muted-foreground shrink-0" />{:else}<ChevronDown class="h-4 w-4 text-muted-foreground shrink-0" />{/if}
                  </button>
                  {#if expandedItems.has(bug.id)}
                    <div class="px-3 pb-3 space-y-2 border-t bg-muted/20">
                      <p class="text-sm text-muted-foreground pt-2"><strong>Fix:</strong> {bug.suggestion}</p>
                      {#if bug.codeSnippet}
                        <div class="relative">
                          <pre class="text-xs p-2 rounded bg-background overflow-x-auto"><code>{bug.codeSnippet}</code></pre>
                          <Button variant="ghost" size="icon" class="absolute top-1 right-1 h-6 w-6" onclick={() => copyCode(bug.codeSnippet!, bug.id)}>
                            {#if copiedId === bug.id}<Check class="h-3 w-3" />{:else}<Copy class="h-3 w-3" />{/if}
                          </Button>
                        </div>
                      {/if}
                    </div>
