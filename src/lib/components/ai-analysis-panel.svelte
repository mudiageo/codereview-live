          ...
          <TabsContent value="bugs" class="mt-4 space-y-3">
            ...
            {:else}
              {#each analysis.bugs as bug}
                <div 
                  class={[ "rounded-lg border overflow-hidden", { "border-red-500/30": bug.severity === 'critical' || bug.severity === 'high'} ]}                
                  >
                  <button
          ...
          <TabsContent value="security" class="mt-4 space-y-3">
            ...
            {:else}
              {#each analysis.securityIssues as issue}
                <div 
                  class={[ "rounded-lg border overflow-hidden", { "border-red-500/30": issue.severity === 'critical' || iesue.severity === 'high'} ]}           
                >
                  <button
                    class="w-full p-3 flex items-start gap-3 text-left hover:bg-muted/30 transition-colors"
                  ...
                  {#if expandedItems.has(suggestion.id) && suggestion.before && suggestion.after}
                    <div class="px-3 pb-3 border-t bg-muted/20 space-y-2">
                      <div class="pt-2">
                        ...
                        <pre class="text-xs p-2 rounded bg-red-500/10 overflow-x-auto"><code>{suggestion.before}</code></pre>
                      </div>
                      <div>
                        <p class="text-xs font-medium text-muted-foreground mb-1">After:</p>
                        <pre class="text-xs p-2 rounded bg-green-500/10 overflow-x-auto"><code>{suggestion.after}</code></pre>
                      </div>
                    </div>
                  {/if}
                </div>
